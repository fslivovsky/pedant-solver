...
class BoolConstraint: public ListConstraint {
 public:
  BoolConstraint(string parameter_name) : ListConstraint({"true","false","1","0"},parameter_name) {}

  virtual string message() {
    return "ERROR: " + parameter_name + " must hold a boolean value";
  }
};
...
class IntRangeConstraint: public RangeConstraint<int> {
public:
  IntRangeConstraint(const int& lower, const int& upper, string parameter_name, bool lower_open=false, bool upper_open=false): RangeConstraint(lower, upper, parameter_name, lower_open, upper_open) {}
  virtual bool check(map<string, docopt::value> args) {
    if (!args[parameter_name]) {
      return true;
    }
    try {
      int value = std::stoi(args[parameter_name].asString());
      return ((lower_open && lower < value) || lower <= value) && ((upper_open && upper > value) || upper >= value);
    } catch (...) {
      return false;
    }
